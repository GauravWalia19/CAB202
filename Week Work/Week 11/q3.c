...
// This will control if the debug message should be displayed of not (without this the timer and button seem to still work during the breakpoint)
bool debug = true;
...
int main() {
	...
	//when this loop is broken out of the teensy is successfully connected 
...
void send_debug_string(char* string) {
	...
	strcpy(buff, "DBG @ %000.00");

	len = strlen(buff);
	sprintf(buff, "DBG @ %06.2f", get_system_time());
    usb_serial_write(buff, len);
...
void enter_breakpoint(unsigned long line_num) {
	...
	int lenn;
	// Send the debug preamble...
	strcpy(buff2, "Entered breakpoint @ line 00. Press b to continue...");
	lenn = strlen(buff2);
	//send_debug_string("Entered breakpoint @ line #. Press b to continue...");
	sprintf(buff2,"Entered breakpoint @ line %ld. Press b to continue...", line_num);
	usb_serial_write(buff2, lenn);
	// Go to a new line (force this to be the start of the line)
	usb_serial_putchar('\r');
	usb_serial_putchar('\n');
