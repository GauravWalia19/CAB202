...
void move_and_draw_balls(int y_array[], int dy_array[]) {
	...
	//iterate through each ball using for loop
	for (int i = 0; i < NUM_BALLS; i++) {
		//if balls have not reached bottom of the screen, increment with dy
		if ((y_array[i] + dy_array[i]) <= screen_height() - 1) {
		y_array[i] += dy_array[i];

		}
		else if ((y_array[i] + dy_array[i]) > screen_height() - 1) {
		y_array[i] = screen_height() - 1;
}
		//dont forget to draw
		draw_char(screen_width() / NUM_BALLS*i, y_array[i], 'o');
